...
public class HzToPinyinUtil {
    ...
    private static HzToPinyinUtil sInstance;
    ...
    protected HzToPinyinUtil(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static HzToPinyinUtil getInstance() {
        synchronized (HzToPinyinUtil.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)
                    ...
                    sInstance = new HzToPinyinUtil(true);
                    return sInstance;
                }
            }
            Log.w(TAG,
                    "There is no Chinese collator, HanziToPinyin is disabled");
            sInstance = new HzToPinyinUtil(false);
            return sInstance;
        }
    }
     ...
     */
    // 输入汉字返回拼音的通用方法函数。
    public String getFirstPinYin(String hanzi) {
