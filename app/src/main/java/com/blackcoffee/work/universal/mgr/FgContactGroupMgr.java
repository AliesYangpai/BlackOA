 ...
 * 版本
 */
public class FgContactGroupMgr {
    /**
     * fragment相关
     */
    ...
    public FgContactGroupMgr() {
    }

    public FgContactGroupMgr(FragmentManager fragmentManager, FragmentTransaction transaction) {
    ...
    public void getDifferentFragment(String fgTag) {
        ...
        if (null == fragment) {
            ...
            if (fgTag.equals(FragmentContactGroupsMyCreate.TAG)) {
                //我创建的
                fragment = new FragmentContactGroupsMyCreate();

            } else if (fgTag.equals(FragmentContactGroupsMyInvited.TAG)) {

                //我加入的
                fragment = new FragmentContactGroupsMyInvited();
            }
            ...
            if (null != currentTopFragment) {
                ...
                transaction.hide(currentTopFragment).add(R.id.fragment_container_contact_group, fragment, fgTag).commit();  //如果存在则让其隐藏【解决创建后隐藏问题】
                currentFgTag = fgTag;
                ...
                transaction.add(R.id.fragment_container_contact_group, fragment, fgTag).commit();
    ...
    public void getDifferentFragmentByPosition(int position) {
        ...
        String fgTag = logicViewFg.getFgContactGroupTagByPosition(position);
    ...
    public void switchFragment(Fragment from, Fragment to) {
        if (!to.isAdded()) {    // 先判断是否被add过
            transaction.hide(from).add(R.id.fragment_container_contact_group, to).commit(); // 隐藏当前的fragment，add下一个到Activity中
        } else {
